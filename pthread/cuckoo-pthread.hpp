...
private:
    ...
    inline int do_hash(const T val, const int func_idx) {   // FUNC_IDX >= 1.
        // cout << "val: " << val << " func_idx: "<< func_idx << endl;
...
CuckooHashTablePthread<T>::lookup_val(const T val) {
    ...
    // cout << "p lookup val "<< val << endl;
    look_args args[_num_funcs+1];
    pthread_t threads[_num_funcs+1];
    bool flag = false;
    for (int i = 1; i <= _num_funcs; ++i) {
        args[i].my_id = i;
    ...
    for(int i = 1; i <= _num_funcs; ++i){
        pthread_join(threads[i], NULL);
        // cout << "p result: " << args[i].result << endl;
        if(args[i].result != -1){
           flag = true;
        }
    }
    // cout << "pthread lookup_val result: " << flag << endl;
    return flag;
}
