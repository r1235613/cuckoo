        ...
        for (int num_funcs = 3; num_funcs <= 4; ++num_funcs) {
            for (int scale = 10; scale <= 24; ++scale) {
                ...
                for (int rep = 0; rep < 5; ++rep) {
                    ...
                    // Pthread.
                    {
                        CuckooHashTablePthread<uint32_t> p_hash_table(size, 4 * ceil(log2((double) n)),
                                                             num_funcs);
                        auto ts = std::chrono::high_resolution_clock::now();
                        int max_levels = 0;
                        for (int i = 0; i < n; ++i) {
                            int levels = p_hash_table.insert_val(vals_to_insert[i], 0);
                            if (levels == ERR_DEPTH) {
                                max_levels = ERR_DEPTH;
                                break;
                            } else if (levels > max_levels)
        ...
        for (int num_funcs = 3; num_funcs <= 4; ++num_funcs) {
            ...
            for (int percent = 0; percent <= 10; ++percent) {
                ...
                for (int rep = 0; rep < 5; ++rep) {
                    ...
                    // pthread
                    {
                        CuckooHashTablePthread<uint32_t> hash_table(size, 4 * ceil(log2((double) n)),
                                                             num_funcs);
                        for (int i = 0; i < n; ++i)
                            hash_table.insert_val(vals_to_insert[i], 0);
                        auto ts = std::chrono::high_resolution_clock::now();
