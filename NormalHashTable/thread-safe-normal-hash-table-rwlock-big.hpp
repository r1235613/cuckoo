...
template<typename T>
class HashTableNormalRwlockBig{
private:
...
public:
    HashTableNormalRwlockBig(size_t size): _size(size){
        _data = new hash_unit[size]();
        counter = 0;
    }
    ~HashTableNormalRwlockBig(){
...
template <typename T>
void HashTableNormalRwlockBig<T>::insert_val(const T val){
...
template <typename T>
bool HashTableNormalRwlockBig<T>::delete_val(const T val){
    uint32_t pos = val % _size;
    std::unique_lock<std::shared_mutex> lock2(g_locks);
    node *cur = _data[pos].next;
    node *prev = nullptr;
    while(cur != nullptr){
        if(cur->data == val){
...
template <typename T>
bool HashTableNormalRwlockBig<T>::lookup_val(const T val){
    uint32_t pos = val % _size;
    //std::lock_guard<std::shared_mutex> lock(g_locks);
    std::shared_lock<std::shared_mutex> lock2(g_locks);
    node *cur = _data[pos].next;
    while(cur != nullptr){
        if(cur->data == val){
            return true;
...
template <typename T>
void HashTableNormalRwlockBig<T>::show_content(){
...
template <typename T>
void HashTableNormalRwlockBig<T>::rehash(){
    std::shared_lock<std::shared_mutex> lock2(g_locks);
